...
```
<mark class="highlight">Use the second cdn <`script`> tag for now.</mark>
...
<mark class="highlight">Eg. `name` will translate to `entry.name` or `${entry.name}` when building your HTML later (description and the other params will follow).</mark>
...
<mark class="highlight">You may add more classes but the main `type`, `id` and `class` *must* stay, this is because our code uses that to select these HTML elements!</mark
...
<mark class="highlight">This is a required step as of now, omitting it will result in referenceError(s)</mark>
...
<mark class="highlight">It's fine if you leave these as blank, they will then follow the default values!</mark>
